...
if selected_option == "Encode Data":
    ...
    with col2:
        ...
        if uploaded_data is not None:
            ...
            if isinstance(uploaded_data, str):
                ...
                uploaded_data = "data/uploaded_data.txt"
                ...
                data_bits = file_to_bits(f"data/uploaded_data{data_extension}")
                st.write(f"Data size in bits: {len(data_bits)} bits")

                st.session_state.uploaded_data_path = f"data/uploaded_data{data_extension}"
    ...
    if uploaded_image is not None and uploaded_data is not None:
        if st.button("Encode"):
            try:
                ...
                cv2.imwrite("data/encoded_image.png", encoded_image)
                st.image("data/encoded_image.png", caption="Encoded Image", width=300)

                st.success("Data successfully encoded!")

                with open("data/encoded_image.png", "rb") as f:
                    st.download_button(
    ...
    if uploaded_image is not None:
        ...
        with open("data/uploaded_image.png", "wb") as f:
            f.write(uploaded_image.getbuffer())
        ...
        if st.button("Decode"):
            try:
                in_img = cv2.imread("data/uploaded_image.png")
                if in_img is None:
                    ...
                    if add_data_bitstream:
                        ...
                        output_path = bitstream_to_file(add_data_bitstream, extension, output_path='data/recovered_file')
